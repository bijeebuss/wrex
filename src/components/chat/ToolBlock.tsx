...
function ToolBlockInner({ tool }: { tool: ToolCallState }) {
  return (
    <details className="my-0.5 rounded border border-gray-200 dark:border-gray-700 overflow-hidden">
      <summary className="flex items-center gap-1.5 px-2 py-0.5 cursor-pointer select-none hover:bg-gray-50 dark:hover:bg-gray-800/50 text-xs">
        {tool.status === 'running' ? (
          <svg
            className="w-3 h-3 animate-spin text-blue-500 shrink-0"
            viewBox="0 0 24 24"
            fill="none"
          >
            <circle
